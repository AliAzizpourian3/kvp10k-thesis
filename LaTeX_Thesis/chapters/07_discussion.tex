...
positive-class weighting. SPADE~\cite{spade} provides a relation formulation
operating over document tokens, while BROS~\cite{bros} combines relative
spatial representations with a SPADE-based relation decoder. Both provide
relation-based context, but neither defines the contiguous-span candidate
reduction used in this thesis.
...
The reconstructed Mistral baseline is stronger than V4 in the official
comparable Regular modes, reaching text F1 of $0.769$ and text+location F1 of
$0.662$, compared with $0.392$ and $0.345$ for V4. Both internal systems are
evaluated on the same 581 prepared pages with the same benchmark evaluator.
On this subset, the reconstructed Mistral scores are also numerically above the
published Regular values of $0.659$ and $0.611$. This must not be interpreted
as evidence that the reconstructed system outperforms the published baseline
under equivalent conditions. The local evaluation covers only pages that
survive PDF retrieval and native-text preparation, PyMuPDF text replaces the
paper's OCR-based input, and the prompt, preprocessing, and fine-tuning
implementation are not identical to the published system.

For Regular text+location matching, Mistral has macro precision $0.661$ and
recall $0.663$, whereas V4 has precision $0.371$ and recall $0.323$. V4's lower
recall is consistent with the qualitative missed-link examples, while its
moderate precision indicates that incorrect predicted links are also an
...
Table~\ref{tab:stage4-cluster} compares the two systems across the
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    \midrule
    Reconstructed Mistral & 0.679 & 0.633 \\
    V4                    & 0.305 & 0.410 \\
    \bottomrule
  \end{tabular}
\end{table}
...
in this thesis. The SPADE token-level dependency graph and the BROS relative
spatial encoding, area-masked pre-training, and SPADE-based decoder nevertheless
provide relevant methodological context for relation-based document extraction.
...
then uses 4{,}851 pages for model training and 538 for validation. The Stage~3
loader separately skips 404 prepared pages with empty targets, so Mistral
training uses 4{,}985 pages, or 51.6\% of the official training split. Some
PDFs were unavailable or broken, and others had no usable text layer. The
pipeline uses PyMuPDF native PDF text extraction rather than OCR, so scanned
pages without a text layer are excluded.

Second, the reconstructed Mistral baseline is a re-implementation rather than
an exact reproduction. Its verified LoRA rank $r=4$ matches the released
configuration, while its prompt/output details, input extraction, and training
coverage differ. Third, V4
