...
Stage~4 training is executed on the TinyGPU cluster using A100~40\,GB
...
\begin{enumerate}
  ...
  \item \textbf{Job 2 -- Stage~4a (entity pre-training):} Single A100~40\,GB
  GPU, 24-hour wall-clock limit, trains LayoutLMv3 encoder + entity head
  with the linker disabled.
  \item \textbf{Job 3 -- Stage~4b (joint fine-tuning):} Single A100~40\,GB
  GPU, 24-hour wall-clock limit, resumes from Stage~4a checkpoint and
  activates the linker with $\lambda > 0$.
\end{enumerate}

All Stage~4 computations use fp32 precision, requiring no
quantisation of the 125M-parameter LayoutLMv3-base model. Peak GPU memory
utilisation during training is approximately 4\,GB (encoder forward pass +
gradient accumulation buffer), well within the 40\,GB envelope.
...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    Linker head & Scaled dot-product scorer \\
    ...
    Precision & fp32 (no quantisation) \\
...
Stage~4 results are presented in Chapter~\ref{ch:results}.
...
problem identified in Stage~3. Results are presented in
...
\subsection{V1 vs.\ V2 Linking Experiment}
\label{sec:v1-v2-experiment}

The central experiment of this thesis compares two linking granularities:

\begin{itemize}
  \item \textbf{V1 (token-level):} Each key token is scored against each
  value token, producing a dense $n_k \times n_v$ matrix with $\sim$450:1
  neg:pos ratio.
  \item \textbf{V2 (span-level):} Contiguous same-label tokens are grouped
  into spans, then each key span is scored against each value span, producing
  a $K \times V$ matrix with $\sim$5:1 neg:pos ratio.
\end{itemize}

Both variants share the same LayoutLMv3 encoder and entity classifier; only
the linker granularity differs. V2 warm-starts from the best V1 checkpoint
to ensure that encoder representations are comparable. Results are reported
in Section~\ref{sec:v1-results} (V1) and Section~\ref{sec:v2-results} (V2).

\subsection{Diagnostic Experiments}
\label{sec:diagnostic-experiments}

After V2 training produced near-zero link F1 despite the span-level
reformulation, a systematic root-cause analysis was conducted through three
diagnostic experiments:

\paragraph{Experiment~D1: Link score distribution.}
A diagnostic script extracted raw sigmoid link scores from the best V2
checkpoint across 200 test documents.  The goal was to determine whether the
linker produces any high-confidence predictions or is uniformly near-zero.

\paragraph{Experiment~D2: Entity span analysis.}
A second diagnostic compared ground-truth entity token counts against
predicted entity token counts, measuring the effect of the bounding-box
validity filter in the span grouping function.  For each document, the script
recorded (i)~predicted key/value tokens before and after \texttt{bbox\_valid}
filtering, (ii)~the number of grouped spans, and (iii)~the word-level and
token-level link label counts.

\paragraph{Experiment~D3: Link label audit.}
The third diagnostic traced the link label generation pipeline from
ground-truth KVPs through word-level labels, token-level alignment, and
span-level collapse.  This revealed the cross-product explosion and the
OCR-block granularity mismatch described in
Section~\ref{sec:text-processing}.

\subsection{V4 Training Configuration}
\label{sec:v4-training}

Based on the diagnostic findings, a corrected V4 training run (job~1568926)
was submitted with the four data-pipeline fixes described in
Section~\ref{sec:stage4-training-loop}.  The hyperparameters are identical
to V2 except:
\begin{itemize}
  \item \textbf{Linker loss weight} $\lambda = 5.0$ (increased from 1.0 to
  compensate for the sparser but correct link supervision signal).
  \item \textbf{Early stopping patience} = 10 (increased from 7 to allow the
  linker more epochs to converge with the corrected labels).
