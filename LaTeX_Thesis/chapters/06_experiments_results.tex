...
The experiments proceed as follows. We begin with a generative baseline
(Mistral-7B) to establish a reference point and characterise the failure
modes of text-only generation on spatial documents. Catastrophic hallucination
on text-sparse layouts motivates a shift to a layout-aware discriminative
model. We then investigate two linking granularities, identify a structural
imbalance problem, and through diagnostic experiments identify and correct
three compounding data-pipeline bugs. Under the released KVP10k benchmark, the
final corrected model reaches regular-KVP F1 of 0.334 (text-only) and 0.309
(text+location), below the corresponding published values of 0.659 and 0.611;
diagnostic analyses localise the remaining gap to relation modeling.
...
directly with the published macro scores. Preprocessing and input-text
differences are described in Section~\ref{sec:data-preprocessing}.
...
\subsection*{Result 1: Competitive text-only F1, but hallucination on text-sparse layouts}
\label{sec:res1}
...
\begin{table}[ht]
  \centering
  \caption{Experiment~1 supplementary error analysis by layout cluster
  (pooled entity-level matching, distinct from the pair-level macro benchmark
  in Table~\ref{tab:exp1-overall}).}
...
over-generation ratio. In this experiment, text-only generation without
spatial grounding is therefore particularly unreliable on sparse pages.
...
\begin{table}[ht]
  \centering
  \caption{Experiment~8: V4 regular-KVP macro F1 by layout cluster (same
  benchmark matching as Table~\ref{tab:exp8-results}).}
...
Text-only performance is similar across clusters, but combined F1 falls from
$0.310$ on text-rich pages to $0.227$ on text-sparse pages. The sparse subset
contains only 60 documents and 16 regular ground-truth links, so this difference
has high variance and should not be generalized. It nevertheless provides no
evidence that sparse layouts are easier for V4; location-sensitive matching is
weaker on that subset.
...
generative system that had not been benchmarked against encoder models. The
generative approach, while competitive overall, fails qualitatively on
text-sparse documents due to unsupported text generation, which token-grounded
discriminative models prevent. Making the discriminative model link key-value pairs proved
non-trivial: token-level linking is defeated by class imbalance, and span-level
linking works only after three latent data-pipeline bugs are diagnosed and
corrected. The final 125M-parameter model reaches regular-KVP macro F1 $=0.334$
for text and $0.309$ for text+location, below the paper's $0.659$ and $0.611$.
Internal diagnostics localise the remaining gap primarily to relation modeling.
