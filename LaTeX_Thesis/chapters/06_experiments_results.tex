...
discriminative model and an honest assessment of where it stands relative to
the published KVP10k benchmark.
...
catastrophic hallucination on text-sparse layouts motivates a shift to a
...
arriving at a final corrected model whose text-only link F1 = 0.342 (NED\,$<0.2$, same threshold as the benchmark) remains substantially below the published KVP10k baseline (0.659), with diagnostic
analyses localizing the remaining gap to relation modeling.
...
requesting a Python list-of-lists output. It is not guaranteed to be
token-identical to the (unpublished) prompt of Naparstek et al.; we reproduce
the format and output contract rather than a verbatim string. The one
documented deviation from the original pipeline is that word text and
coordinates are obtained from PyMuPDF's native text layer rather than Tesseract
OCR.
...
\subsection*{Result 1: Competitive Performance, but Spatial Hallucination on Text-Sparse Layouts}
\label{sec:res1}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    0 (text-rich)   & 521 & 58.6 & 41.4 & 32.5 \\
    1 (text-sparse) &  60 &  3.0 & \textbf{97.0} & 38.1 \\
    \bottomrule
  \end{tabular}
\end{table}

On text-sparse documents (pages with very few extractable OCR tokens), the
model hallucinates almost continuously: \textbf{97\%} of its predictions on
text-sparse pages have no correspondence in the actual document. Layout clusters here are the
\emph{actual} Stage~2
...
per page in text-sparse documents, the model over-generates (1{,}301 predicted
...
\subsection*{Result 8: Link F1 = 0.342 (text-only) / 0.309 (text+location) --- Below the KVP10k Baseline}
\label{sec:res8}

\begin{table}[ht]
  \centering
  \caption{Experiment~8: V4 corrected model results. Link F1 is reported under
  both the text-only metric (NED $<0.2$, the same threshold as the published
  baseline) and the stricter text+location metric (NED $<0.2$, IoU $>0.3$).}
  \label{tab:exp8-results}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    \textbf{Model} & \textbf{Entity F1} & \textbf{Link F1} \\
    \midrule
    V4 (corrected, text-only)             & 0.827 & \textbf{0.342} \\
    V4 (corrected, text+location)         & 0.827 & 0.309 \\
    KVP10k paper baseline (Naparstek et al., 2024; text-only) & --- & 0.659 \\
    \bottomrule
  \end{tabular}
\end{table}

With all three bugs corrected, the V4 model achieves link F1 = 0.342 under the
text-only metric (NED\,$<0.2$, identical to the published benchmark's threshold),
and 0.309 under the text+location metric (NED\,$<0.2$, IoU\,$>0.3$). This does
not reach the published KVP10k baseline (0.659, text-only) --- a gap of 0.317
points --- and is obtained with a 125M-parameter discriminative model trained on
55\% of the available training data. While the model does not surpass the
proprietary generative baseline, it demonstrates that encoder-based models are
substantially more capable at KVP linking than the KVP10k paper assumed. Two
diagnostic analyses (Section~\ref{sec:linker-diagnostics}) characterize the
remaining gap.
...
\paragraph{Per-cluster breakdown: the remaining gap is still layout-sensitive.}
\label{sec:stage4-per-cluster}

\begin{table}[ht]
  \centering
  \caption{Experiment~8: V4 per-cluster results on the test set (same matching as Table~\ref{tab:exp8-results}). Cluster assignments are the Stage~2 layout clusters propagated to Stage~4.}
  \label{tab:exp8-cluster}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    \textbf{Cluster} & \textbf{Docs} & \textbf{Entity F1} & \textbf{Link F1 (text-only)} & \textbf{Link F1 (text+location)} \\
    \midrule
    0 (text-rich)   & 521 & 0.828 & 0.341 & 0.309 \\
    1 (text-sparse) &  60 & 0.723 & 0.467 & 0.333 \\
    \bottomrule
  \end{tabular}
\end{table}

The Stage~4 model remains strongest on the text-rich cluster, which dominates the test set and therefore drives the overall score. The text-sparse cluster is small (60 documents, 16 ground-truth links), so its higher apparent link F1 should be read cautiously; the main conclusion is that the corrected linker now performs consistently across both clusters instead of collapsing entirely on one of them.
...
Table~\ref{tab:oracle-linking} reports the result. Perfect entities raise link
F1 by only $\sim$0.02--0.05 in every setting; even with flawless spans the
linker reaches just 0.345 (text+location) / 0.365 (text-only). The entity
detector is therefore \emph{not} the dominant limitation --- the linker
(relation modeling) is.

\begin{table}[ht]
  \centering
  \caption{Analysis~B: end-to-end vs.\ oracle-entity linking on the test set
  (V4 model). Oracle linking uses ground-truth entity spans, isolating linker
  quality from entity detection.}
  \label{tab:oracle-linking}
  \begin{tabular}{@{}lcc@{}}
    \toprule
    \textbf{Setting} & \textbf{End-to-end} & \textbf{Oracle entities} \\
    \midrule
    Link F1 (text-only, NED$<$0.2)                    & 0.342 & 0.365 \\
    Link F1 (text+location, NED$\le$0.2, IoU$\ge$0.3) & 0.309 & 0.345 \\
    \bottomrule
  \end{tabular}
\end{table}

Together, Analyses~A and~B establish that the residual gap to the published
...
\begin{table}[ht]
  \centering
  \caption{Full experimental progression. For V4, link F1 is reported as
  text-only / text+location.}
  ...
  \begin{tabular}{@{}lccl@{}}
    ...
    Exp 1: Mistral-7B (text+loc) & --- & 0.600 & 97\% hallucination on text-sparse layouts \\
    ...
    Exp 8: V4 (corrected)     & 0.827 & 0.342 / 0.309 & Below baseline; linker is the bottleneck \\
    \bottomrule
  \end{tabular}
\end{table}
...
qualitatively on text-sparse documents due to hallucination -- a problem that
...
model, with the right supervision, becomes substantially competitive at KVP
linking (text-only link F1 = 0.342), though it does not surpass the large
generative baseline (0.659). Diagnostic analyses localize the remaining gap to
relation modeling rather than entity detection.
