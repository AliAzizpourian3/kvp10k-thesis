...
\begin{table}[ht]
  \centering
  \caption{Experiment~1 error analysis by layout cluster.}
  \label{tab:exp1-cluster}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    \textbf{Cluster} & \textbf{Docs} & \textbf{Correct (\%)} & \textbf{Hallucination (\%)} & \textbf{Missed (\%)} \\
    \midrule
    0 (Dense)  & 521 & 48.7 & 40.7 & 31.3 \\
    1 (Sparse) &  60 &  2.7 & \textbf{54.4} & 54.5 \\
    \bottomrule
  \end{tabular}
\end{table}

On sparse-layout documents (receipts, simple forms), the model hallucinates
over half the time: it generates KVP predictions that have no correspondence
in the actual document. Layout clusters here are the \emph{actual} Stage~2
...
per page in sparse documents, the model over-generates (667 predictions vs.\
44 ground-truth for Cluster~1), producing a $15.2\times$ over-generation
ratio. This is a
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lccl@{}}
    ...
    Exp 1: Mistral-7B (text+loc) & --- & 0.600 & 54.4\% hallucination on sparse layouts \\
