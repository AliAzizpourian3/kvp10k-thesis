...
(Figure~\ref{fig:stage2_tsne_umap}). Unlike the linear PCA above (standardized
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{figures/stage2/tsne_clusters.png}
    \hfill
    \includegraphics[width=0.48\textwidth]{figures/stage2/umap_clusters.png}
    \caption{Non-linear projections of the $k=2$ layout clusters: t-SNE on a
    stratified 3{,}000-page subsample (left) and UMAP on all 25{,}692 pages
    (right). Features are $\log(1+x)$-transformed, robust-scaled, and jittered
    ($\sigma=0.01$); the sparse cluster's spread is a jitter artifact, not real
    structure. Both reproduce the separation observed in the PCA projection.}
    \label{fig:stage2_tsne_umap}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lrrrrrr@{}}
        \toprule
        \textbf{Cluster} & \textbf{$n_{\text{boxes}}$ mean} & \textbf{$n_{\text{boxes}}$ std}
            & \textbf{coverage mean} & \textbf{coverage std}
            & \textbf{mean\,area mean} & \textbf{mean\,area std} \\
        \midrule
        0 (text-rich)   & 25.3622 & 20.5338 & 0.0582 & 0.0467 & 0.0029 & 0.0030 \\
        1 (text-sparse) &  0.0619 &  0.6941 & 0.0001 & 0.0009 & 0.0000 & 0.0002 \\
        \bottomrule
    \end{tabular}
\end{table}
\begin{itemize}
    \item \textbf{PC1} (71.8\% variance): Captures overall content density (the number of extractable text boxes and their coverage); this dominant axis separates the text-rich cluster from the text-sparse cluster.
...
We store the assigned layout cluster ID for each page and use it as a stratification variable in later evaluation. In Stages~3--4 we report overall benchmark performance as well as \emph{per-cluster} performance to identify whether specific layout types remain challenging (see Section~\ref{sec:stage3-per-cluster}).
