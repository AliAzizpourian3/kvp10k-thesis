...
spans before pairwise scoring. The span-level approach draws its segment-level motivation from
SPADE~\cite{spade}. BROS~\cite{bros} provides complementary motivation through
sparse linking over entity-initial positions and spatially informed
representations (Section~\ref{subsec:entity_linking}).
...
at the \emph{span} level rather than the token level. SPADE~\cite{spade}
provides the segment-level motivation, while BROS~\cite{bros} motivates sparse
entity-initial linking and spatially informed representations.
