...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    % Span grouper (V2 and V4)
    \node[block, below=0.8cm of entity, fill=green!12] (spans) {Span Grouper\\(V2 and V4)};
  ...
  \caption{Overview of the proposed LayoutLMv3 architecture with a projected scaled-dot-product linker.
    ...
    (pairwise key--value scoring). In V2 and V4, a span grouper merges contiguous
...
bounding-box overlap ratios, relative area ratio, and the ratio of aspect
ratios. These are encoded by a two-layer MLP with ReLU activations into a
...
% ---------------------------------------------------------------------------
\section{Span-Level Linker (V2/V4)}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\tiny, above=0.15cm of t4] {VALUE};
    \node[font=\tiny, above=0.15cm of t5] {VALUE};
    \node[font=\tiny, above=0.15cm of t6] {VALUE};
    \node[font=\tiny, above=0.15cm of t7] {KEY};
    \node[font=\tiny, above=0.15cm of t8] {KEY};
    \node[font=\tiny, above=0.15cm of t9] {VALUE};
...
The span-level projected scaled-dot-product scorer operates identically to V1
...
\subsection{Linking (V2/V4)}
\label{sec:v2-linking-inference}

In V2 and V4, spans are first formed by the grouping step
(Section~\ref{sec:span-grouping}), then span representations are computed via
mean-pooling. The span-level projected scaled-dot-product scorer produces a $K \times V$
