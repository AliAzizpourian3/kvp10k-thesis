...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    % Input
    \node[io] (input) {OCR Words +\\Bounding Boxes\\{\scriptsize Page image optional; not used in V4}};
...
its parent OCR word. Multi-word entity and span boxes are formed later as the
...
the text tokens when document-image input is enabled.

Although LayoutLMv3 supports document-image inputs, the final V4 launch did not
enable page-image loading. The data pipeline supplied blank white placeholder
pixel tensors rather than actual page pixels. The reported V4 results therefore
use text and layout information only, with no page-image content.
...
poor spatial grounding and systematic unsupported generation
(Section~\ref{sec:stage3-per-cluster}). Token classification is grounded by
construction: predictions are tied to OCR tokens with known bounding boxes.
This prevents the model from generating text absent from the OCR input,
although it can still assign an incorrect entity label to an existing token.
...
dot-product formulation avoided this failure in subsequent runs.

The module is referred to as the Biaffine Linker for continuity with the
implementation and early design. However, the final V4 scorer uses projected
scaled dot-product similarity combined with spatial features, rather than a
full bilinear biaffine matrix.
...
BCE loss, the extreme imbalance in V1 was associated with failure to learn
reliable associations. The resulting legacy pooled diagnostic link F1 was
approximately $0.008$ (see
Section~\ref{sec:res3}).
...
span and one value span respectively. During linker training, spans are formed
from ground-truth entity labels through teacher forcing. At inference, spans
are formed from the entity classifier's predicted labels.
...
\begin{itemize}
  ...
  swept $\{0.5, 1.0, 2.0\}$ (see Section~\ref{sec:exp2}); final V4 used
  $\lambda=5.0$.
\end{itemize}
...
linear warmup of 500 steps followed by linear decay to zero. For the final V4
run, training used batch size 1 with gradient accumulation over 8 steps
(effective batch size 8), a unified learning rate of $2\times10^{-5}$, and
early stopping with patience 10 based on validation entity F1. The best
checkpoint was selected by validation entity F1, not linker F1. Training
targeted 30 epochs across resumed jobs; the final run history does not support
interpreting this as a single uninterrupted 30-epoch run.
...
reverting to fp32 resolved the issue.
...
\begin{enumerate}
  ...
  development runs followed the Stage~4a entity checkpoint; final V4 is the
  corrected V2 pipeline and was warm-started from the prior
  \texttt{stage4b\_canary\_B} checkpoint rather than trained from scratch.
  Its trainable parameters used the unified $2\times10^{-5}$ learning rate.
\end{enumerate}
...
mean-pooling. The span-level projected dot-product scorer produces a $K \times V$ score
matrix and the highest-scoring match for each key span is selected.

\subsection{Post-processing}
\label{sec:post-processing}

The implemented decoder emits linked key--value span pairs above the fixed
link-score threshold. Unmatched key and value spans are not converted into
\emph{unvalued} or \emph{unkeyed} predictions; consequently, the discriminative
experiments directly cover the regular-KVP category of the KVP10k benchmark,
not its two singleton categories. No beam search or constrained decoding is
applied. Bounding boxes are inherited directly from the OCR input. A separate
decode-time recovery ablation emits unmatched predicted spans as unkeyed or
unvalued outputs; it is reported separately and is not part of the primary
regular-KVP result.
