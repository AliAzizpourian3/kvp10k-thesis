...
class LayoutLMv3PreparedDataset(Dataset):
    ...
    def __init__(
        ...
        self.processor = processor if processor else LayoutLMv3Processor.from_pretrained(
            "microsoft/layoutlmv3-base",
            apply_ocr=False  # We provide pre-extracted text + bboxes, no OCR needed
        )
        self.max_seq_length = max_seq_length
        self.include_images = include_images
        self.image_base_dir = image_base_dir
        
        # Load all JSON file paths
    ...
    def __getitem__(self, idx):
        ...
        # Parse lmdx_text format: "word1 x1|y1|x2|y2\nword2 x1|y1|x2|y2"
        ...
        # Normalize bboxes to [0, 1000] scale (LayoutLMv3 standard)
        ...
        # If no image available, create a dummy image for LayoutLMv3Processor
        # (processor requires images parameter even with apply_ocr=False)
        if image is None:
            # Create a blank grayscale dummy image
            image = Image.new('RGB', (224, 224), color=(255, 255, 255))
        
        # Generate entity labels (0=Other, 1=Key, 2=Value) and link labels
        gt_kvps = data.get('gt_kvps', {}).get('kvps_list', [])
        entity_labels, link_labels = self._generate_labels(
            words, 
            gt_kvps,
            word_bboxes,  # Pass pixel bboxes for bbox overlap matching
            image_width,
            image_height
        )
        
        # Tokenize with LayoutLMv3Processor
        try:
        ...
        # Get word_ids for word-to-token alignment
        # word_ids[i] gives the word index for token i (or None for special tokens)
        word_ids = encoded.word_ids()
        
        # Align word-level entity labels to token level
        ...
        # Add pixel values (required by LayoutLMv3)
    ...
    def _parse_lmdx_text(self, lmdx_text: str, data: Dict) -> Tuple[List[str], List[List[int]]]:
        ...
        for line in lines:
            ...
            # Split on last space to separate word from coordinates
            parts = line.rsplit(' ', 1)
            if len(parts) != 2:
                continue
            ...
            # Parse bbox: x1|y1|x2|y2 (pixel coordinates)
            try:
    ...
    def _align_labels_to_tokens(
        ...
        word_ids[token_idx] gives word_idx for that token.
        Special tokens (CLS, SEP, PAD, etc.) have word_idx = None.
        """
    ...
    def _align_link_labels_to_tokens(
        ...
        """
        Align word-level link labels to token level using word_ids.
        
        Converts a word-level link matrix [max_seq, max_seq] to token-level
        by mapping word indices through word_ids.
        
        Args:
            word_link_labels: [max_seq, max_seq] word-level adjacency matrix
            word_ids: [num_tokens] mapping from token idx to word idx
        
        Returns:
            token_link_labels: [max_seq, max_seq] token-level adjacency matrix
        """
        max_len = word_link_labels.shape[0]
        token_link_labels = torch.zeros((max_len, max_len), dtype=torch.float32)
        
        # Map token pairs through word_ids
        for token_i, word_i in enumerate(word_ids):
            ...
            for token_j, word_j in enumerate(word_ids):
                if word_j is None or token_j >= max_len:
                    continue
                # Copy word-level link to token-level
    ...
    def _generate_labels(
        ...
        Uses bbox overlap matching for more accurate labeling than text matching.
        
        Entity labels: 0=Other, 1=Key, 2=Value
        Link labels: [seq_len x seq_len] binary adjacency matrix where
                     link_labels[i, j] = 1 if word i is a key and word j is its value
        """
        ...
        for kvp in kvps:
            ...
            # Find word indices by bbox overlap
            ...
            for idx in key_indices:
                if idx < len(entity_labels):
                    entity_labels[idx] = 1  # Key
            for idx in val_indices:
                if idx < len(entity_labels):
                    entity_labels[idx] = 2  # Value
            
            # Create link labels: connect all key words to all value words
    ...
    def _find_words_by_bbox_overlap(
        ...
        """
        Find word indices that have bbox overlap with target_bbox.
        
        Uses intersection area to determine relevance.
        """
        ...
        for idx, bbox in enumerate(word_bboxes):
            ...
            if inter_x1 < inter_x2 and inter_y1 < inter_y2:
                ...
                # Only match if overlap is significant (>50% of target)
    ...
    def _get_empty_example(self):
        """Return a padding example for empty/invalid data."""
        return {
...
class PaddedBatchCollator:
    ...
    def __call__(self, batch):
        ...
        for key in keys:
            if key == 'hash_name':
                # Keep hash names as list of strings
                collated[key] = [item[key] for item in batch]
            elif key in ['input_ids', 'attention_mask', 'entity_labels']:
                # Stack 1D tensors
                collated[key] = torch.stack([item[key] for item in batch])
            elif key == 'bbox':
                # Stack 2D tensors [seq_len, 4]
                collated[key] = torch.stack([item[key] for item in batch])
            elif key == 'pixel_values':
                # Stack image tensors [3, 224, 224]
                collated[key] = torch.stack([item[key] for item in batch])
            elif key == 'link_labels':
                # Stack link label matrices [seq_len, seq_len]
                collated[key] = torch.stack([item[key] for item in batch])
...
def create_stage4_dataloaders(
    ...
    Args:
        ...
        val_fraction: Fraction of training data to use for validation (e.g., 0.1)
        num_workers: DataLoader workers
        include_images: Whether to include image pixel values
    ...
    processor = LayoutLMv3Processor.from_pretrained(
        "microsoft/layoutlmv3-base",
        apply_ocr=False  # We provide pre-extracted text + bboxes
    )
