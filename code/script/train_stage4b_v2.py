"""
Stage 4b V2 Training: LayoutLMv3 KVP Extraction with Span-Level Biaffine Linker.

Identical to train_stage4b.py except it imports and uses LayoutLMv3KVPModelV2
from layoutlm_model_v2.py, which groups contiguous same-label tokens into
spans before linking. All hyperparameters, loss weights, and training loop
logic are unchanged.
"""
...
class Stage4bV2Trainer:
    """Trainer for Stage 4b V2: LayoutLMv3 with Span-Level Biaffine Linker."""
    ...
    def __init__(
        ...
        logger.info(f"Model version: V2 (span-level linker)")
    ...
    def train_epoch(self):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            num_batches += 1
            e_l = entity_loss.item() if entity_loss is not None else 0.0
    ...
    def train(self):
        logger.info(f"Starting Stage 4b V2 training | linker_loss_weight={self.linker_loss_weight}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Stage 4b V2 Training: Span-Level Linker")
    parser.add_argument("--data_dir",                    type=str,   default="../../data/prepared")
    parser.add_argument("--output_dir",                  type=str,   default="../../data/outputs/stage4b_v2")
    ...
    logger.info("LayoutLMv3KVPModelV2 (span-level linker) created on CPU")
    ...
    if args.pretrained_encoder and not latest_ckpt:
        ...
        if pretrained_path.exists():
            ...
            lnk_keys = [k for k in transferred if k.startswith('linker.')]
            logger.info(f"\u2713 Transferred {len(transferred)} keys on CPU (skipped {len(skipped)})")
            logger.info(f"  Encoder keys: {len(enc_keys)}, Classifier keys: {len(cls_keys)}, Linker keys: {len(lnk_keys)}")
            if len(enc_keys) == 0:
