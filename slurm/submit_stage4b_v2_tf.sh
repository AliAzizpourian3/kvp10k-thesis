#!/bin/bash
#SBATCH --job-name=kvp_4b_v2tf
#SBATCH --output=logs/kvp_stage4b_v2tf-%j.out
#SBATCH --error=logs/kvp_stage4b_v2tf-%j.err
...
echo "=== Stage 4b V2 + Teacher Forcing (span-level linker) ==="
...
if [[ ! -d "${HUGGINGFACE_HUB_CACHE}/models--microsoft--layoutlmv3-base" ]]; then
  echo "ERROR: LayoutLMv3 not found in HF cache"
  exit 1
fi
...
echo "Fix: teacher forcing (GT entities for linker during training)"

"$PYTHON_BIN" code/script/train_stage4b_v2.py \
  --data_dir            data/prepared \
  --output_dir          data/outputs/stage4b_v2_tf \
  --batch_size          1 \
