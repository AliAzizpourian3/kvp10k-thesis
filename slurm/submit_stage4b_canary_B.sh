#!/bin/bash
#SBATCH --job-name=kvp_4b_canary_B
#SBATCH --output=logs/kvp_stage4b_canary_B-%j.out
#SBATCH --error=logs/kvp_stage4b_canary_B-%j.err
...
echo "=== Stage 4b CANARY B (conservative: lr=2e-5, accum=8) ==="
...
"$PYTHON_BIN" code/script/train_stage4b.py \
  ...
  --output_dir          data/outputs/stage4b_canary_B \
  --batch_size          1 \
