...
\begin{enumerate}[noitemsep]
  ...
  \item Bounding-box coordinates kept in original pixel space for
        traceability; normalisation to $[0, 1000]$ performed later
        (Stage 3) as required by LayoutLMv3.
\end{enumerate}

\subsection*{Output}
PNG images and per-page metadata files stored under
\texttt{data/raw/}.
...
newline-delimited sequence of such lines and stored in the field
\texttt{lmdx\_text}.

\subsection*{Prepared JSON Schema}
Each output file contains the following top-level fields:
...
\begin{table}[h!]
  ...
  \begin{tabularx}{\textwidth}{llX}
    ...
    \texttt{lmdx\_text}      & string  & Words with pixel-space bounding boxes
                                         ...
                                         \texttt{word x1|y1|x2|y2}) \\
    \texttt{image\_width}    & int     & Page width in pixels \\
    \texttt{image\_height}   & int     & Page height in pixels \\
    \texttt{gt\_kvps}        & object  & Ground-truth KVPs;
                                         contains \texttt{kvps\_list} --- a
                                         list of entries each with
                                         \texttt{type}, \texttt{key.text},
                                         \texttt{key.bbox},
                                         \texttt{value.text},
                                         \texttt{value.bbox} \\
    \bottomrule
...
\begin{table}[h!]
  \centering
  \caption{Prepared dataset split statistics}
  \label{tab:prepared}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Split} & \textbf{JSON files} & \textbf{Total KVP entries}
    & \textbf{Valid \texttt{kvp} pairs} & \textbf{Unvalued / Unkeyed} \\
    \midrule
    Train & 5{,}389 & 81{,}817 & 35{,}363\;(43.2\,\%) & 42{,}157\;(51.5\,\%) \\
    Test  & $\sim$600 & --- & --- & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\noindent
An important data characteristic surfaced during preprocessing analysis:
only $43.2\,\%$ of annotated entries are complete \texttt{kvp} pairs
(both key and value present with non-empty text and non-degenerate
bounding boxes).
The remaining $56.8\,\%$ are either \texttt{unvalued} keys or
\texttt{unkeyed} values --- partial annotations that are valid for
entity labelling but must not generate key-value link targets.
This distinction is handled explicitly in the data preparation step for
Stage 4 (see Section~\ref{sec:stage4}).
...
parameters) on the prepared KVP10k data to perform end-to-end key-value
pair extraction using a two-component architecture.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Biaffine Linker} --- pairs key tokens to value tokens using
        a scaled dot-product semantic score combined with an 8-feature
...
\begin{itemize}[noitemsep]
  \item \textbf{Entity F1} --- precision/recall on Key/Value token
        classification.
  \item \textbf{KVP F1} --- end-to-end extraction metric: a predicted pair
        is correct if the key text matches (NED $< 0.2$) and the bounding
        boxes overlap (IoU $> 0.3$).
...
\begin{itemize}[noitemsep]
  \item \textbf{Data preprocessing complexity} --- the distinction between
        ...
        all types, but link labels only for complete \texttt{kvp} entries
        with non-empty text and non-degenerate bounding boxes.
  \item \textbf{GPU memory constraints} --- the 512-token limit and
        $512 \times 512$ link label matrix impose strict batch-size
        restrictions on the 40\,GB A100; chunked linker scoring and
        candidate capping (max 128 keys / 128 values per page) are
        applied to stay within budget.
